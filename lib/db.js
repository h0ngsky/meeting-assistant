...
async function getMeetings() {
  const data = await readStore('meetings', { meetings: [], nextId: 1 });
  const { toWallClockString, hasTimezone } = require('./rooms');
  let changed = false;
  for (const m of data.meetings) {
    if (m.startTime && hasTimezone(m.startTime)) {
      m.startTime = toWallClockString(m.startTime);
      changed = true;
    }
    if (m.endTime && hasTimezone(m.endTime)) {
      m.endTime = toWallClockString(m.endTime);
      changed = true;
    }
  }
  if (changed) await saveMeetings(data);
  return data;
}
